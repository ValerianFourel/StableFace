 ...
 #    config = UNet2DConditionModel.load_config(args.model_config_name_or_path)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            text_input = tokenizer(batch["text"], padding="max_length", truncation=True, return_tensors="pt", device=clean_images.device)#.to(accelerator.device)
            text_embeddings = text_encoder(text_input.input_ids)[0]
            # Prepare latents
            latents = torch.randn((batch["image"].shape[0], model.in_channels, 64, 64))
            latents = latents.to(accelerator.device)
            with accelerator.accumulate(model):
                # Predict the noise residual
                model_output = model(latents, timesteps,  encoder_hidden_states=text_embeddings)
